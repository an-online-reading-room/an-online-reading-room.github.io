...
import * as api from "$lib/api"
    ...
    const getModalVisited = (name) => {
		return JSON.parse($popup)[name] 
	}
	const setModalVisited = (name) => {
        console.log("here")
    ...
    const enableLinkingMode = () => {
        ...
        if($user.jwt) {
            linkingMode = true
            if(getModalVisited('linkingModal') === false) {
                linkingModal = true
                setModalVisited('linkingModal')
            }
        } else {
            loginModal = true
        }
...
z-50">
    <nav class="flex justify-between stroke-primary h-6">
...
<Modal isOpenModal={linkingModal} on:closeModal={() => linkingModal = false}>
